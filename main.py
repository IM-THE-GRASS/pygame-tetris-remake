...
class peice:
    ...
    def draw_at(self, pos):
        for square in self.shape:
            print(square)
            offset_x = I[0][0] + pos[0]
            print(offset_x)
            #square["color"] = (255,0,255)
            #square["state"] = 0
...
for line in range(lines):
    for i in range(columns):
        ...
        sq.topleft = (board.topleft[0] + gap + ((i * gap) + (i * sq_width)), board.topleft[1] + gap + ((line * gap) + (line * sq_width))) 
        
        squares[(line, i)] = {"rect": sq, "color": (255,255,255), "state": 0}
while running:
    ...
    peices = peice(I)
    peices.draw_at((0,0))
    squares[(0,1)]["color"] = (244,24,42)
    squares[(0,5)]["color"] = (244,24,42)
